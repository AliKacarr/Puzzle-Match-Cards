...
namespace Puzzle_Match_Cards
{
    public partial class Zor : Form
    {
        ...
        public Zor(int level, string username)
        {
            InitializeComponent();
            InitializeSoundPlayers();
            InitializebilgiPanel();
        ...
        private void InitializeGame()
        {
            ...
            visiblePictureBoxes = new List<PictureBox>
            {
                ...
                pictureBox46, pictureBox47, pictureBox48, pictureBox49, pictureBox50,
        ...
        private void AssignImagesToPictureBoxes()
        {
            ...
            List<string> selectedImages = imagePaths.OrderBy(x => random.Next()).Take(15).ToList();
                    ...
                    if (firstClicked.Tag == secondClicked.Tag)
                    {
                        ...
                        if (visiblePictureBoxes.All(p => p.Visible) && textBox2.Text != "0")
                        {
                            ...
                            if (form1 != null && form1 is Form1)
                            {
                                ...
                                if (level != 10)
                                {
                                    form1Instance.SetVisiblePictureBox(level + 51);
                                }
                                    if (pictureBox66.Visible)
                                {
                                    form1Instance.SetStars((level + 80).ToString(), 3);
                                    starCount = 3;
                                }
                                else if (pictureBox65.Visible)
                                {
                                    form1Instance.SetStars((level + 80).ToString(), 2);
                                    starCount = 2;
                                }
                                else if (pictureBox64.Visible)
                                {
                                    form1Instance.SetStars((level + 80).ToString(), 1);
                                    starCount = 1;
                                }
                            }
                            if (username != "username")
                            {
                                int earnedScore = 200 - Convert.ToInt32(originalTime - timeLeft) * 2;
                                UpdateUserScore(username, earnedScore);
                                SetButton1(PuanAl(username).ToString());
                                Form1 form1Instance = (Form1)form1;
                                form1Instance.SetPuanValue(PuanAl(username).ToString());
                                UpdateZorSeviye(level + 1);
                                UpdateZorStar(username, level, starCount);
        ...
        private void UpdateZorSeviye(int newLevel)
        {
            if (newLevel == 11)
            {
                return;
            ...
            // Mevcut ZorSeviye'yi al
            OleDbCommand selectCommand = new OleDbCommand("SELECT ZorSeviye FROM Register WHERE KullaniciAdi = @username", connection);
            ...
            {
                OleDbCommand updateCommand = new OleDbCommand("UPDATE Register SET ZorSeviye = @newLevel WHERE KullaniciAdi = @username", connection);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Form form1 = Application.OpenForms["Form1"];
                Form1 form1Instance = (Form1)form1;
                if (!form1Instance.IsPictureBoxVisible(level + 51) && level != 10)
                {
                    // pictureBox68 görünürlüğünü aç
                    pictureBox68.Visible = true;
                }
            }
        }
        ...
        private void InitializebilgiPanel()
        {
            ...
            Label copyLabel = new Label
            {
                ...
                Location = new Point((bilgiPanel.Width - TextRenderer.MeasureText(Text, new Font("Arial", 10)).Width) - 227, (bilgiPanel.Height - TextRenderer.MeasureText(Text, new Font("Arial", 10)).Height) / 2 - 4),
            };
        ...
        private void pictureBox67_Click(object sender, EventArgs e)
        {
            ...
            if (!isHintActive)  // Eğer ipucu aktif değilse
            {
                ...
                if (currentScore >= 50)  // Kullanıcının puanı 50'den büyükse
                {
                    ...
                    // Tüm eşleşmeyen kartları göster ve 1 saniye sonra gizle
                    ToggleVisibilityOfAllCards();
                    isHintActive = true;  // İpucu aktif durumda

                    // Timer başlat ve 1 saniye sonra resimleri gizle
                    timer3.Start();
                }
                else
                {
        ...
        {
            isMouseOverpictureBox67 = true;
            timer4.Start();
        }

        private bool isMouseOverpictureBox67 = false;

        private void pictureBox67_MouseLeave(object sender, EventArgs e)
        {
            isMouseOverpictureBox67 = false;
            timer4.Stop();
        ...
        private void timer5_Tick(object sender, EventArgs e)
        {
            ...
            if (dolulukOrani <= 50 && dolulukOrani >= 45 && pictureBox66.Visible)
            {
                pictureBox66.Visible = false;
            }
            else if (dolulukOrani <= 30 && dolulukOrani >= 25 && pictureBox65.Visible)
            {
        ...
        private void UpdateZorStar(string username, int level, int starCount)
        {
            try
            {
                connection.Open();
                OleDbCommand command = new OleDbCommand("SELECT ZorStar FROM Register WHERE KullaniciAdi = @username", connection);
                command.Parameters.AddWithValue("@username", username);

                string currentZorStar = (string)command.ExecuteScalar();
                if (currentZorStar != null && currentZorStar.Length > level - 1)
                {
                    char currentStar = currentZorStar[level - 1];
                    ...
                    char[] zorStarChars = currentZorStar.ToCharArray();
                    zorStarChars[level - 1] = newStar;
                    string newZorStar = new string(zorStarChars);

                    // Güncellenmiş değeri veritabanına kaydet
                    OleDbCommand updateCommand = new OleDbCommand("UPDATE Register SET ZorStar = @newZorStar WHERE KullaniciAdi = @username", connection);
                    updateCommand.Parameters.AddWithValue("@newZorStar", newZorStar);
                    updateCommand.Parameters.AddWithValue("@username", username);
                    updateCommand.ExecuteNonQuery();
                }
            }
            ...
            {
                MessageBox.Show("Error updating ZorStar: " + ex.Message);
            }
            finally
            {
                connection.Close();
